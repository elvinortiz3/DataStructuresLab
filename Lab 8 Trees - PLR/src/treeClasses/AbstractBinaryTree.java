...
public abstract class AbstractBinaryTree<E> extends AbstractTree<E> implements
	...
	protected void recDisplay(Position<E> r, int[] control, int level) {
		...
		if(parent == null){ System.out.println("_ROOT(" + r.getElement() + ")"); }
		else if(left(parent) == r){ System.out.println("_L(" + r.getElement() + ")"); }
		else { System.out.println("_R(" + r.getElement() + ")"); }
		control[level]--;
		int nc = this.numChildren(r);
		control[level+1] = nc;
		for (Position<E>  p : this.children(r)) { recDisplay(p, control, level+1); }       
	}
