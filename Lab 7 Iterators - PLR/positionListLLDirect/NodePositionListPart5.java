 ...
 *
 */
public class NodePositionListPart5<T> implements PositionList<T> {

	private DNode<T> header, trailer; 
	private int count; 
	private IteratorMaker<T> iter;
	
	public NodePositionListPart5(IteratorMaker<T> a) {
	...
	public NodePositionListPart5() {
		header = new DNode<T>(null, null, null, this); 
		trailer = new DNode<T>(null, header, null, this);
		header.setNext(trailer); 
		count = 0; 
		iter = new ForwardIteratorMaker<T>();
	}
	...
	public Iterable<Position<T>> positions() {
		NodePositionListPart5<Position<T>> pList = new NodePositionListPart5<Position<T>>(); 
		try { 
			if (!this.isEmpty()) { 
	...
	private static class DNode<T2> implements Position<T2> {
        ...
        private NodePositionListPart5<T2> myList;     // list it belongs to
        
        public DNode(T2 e, DNode<T2> p, DNode<T2> n, NodePositionListPart5<T2> list) { 
        	element = e; 
        	next = n; 
        	prev = p; 
        	myList = list; 
		...
		public void setList(NodePositionListPart5<T2> n) { 
			myList = n; 
		}
		public NodePositionListPart5<T2> getList() { 
			return myList; 
		}
