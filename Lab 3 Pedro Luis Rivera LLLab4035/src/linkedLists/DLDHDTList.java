...
public class DLDHDTList<E> implements LinkedList<E> {
	...
	public Node<E> getNodeAfter(Node<E> target) throws NodeOutOfBoundsException {
		if(length == 0){ throw new NodeOutOfBoundsException("The list is empty."); }
		if(((DNode<E>)target).getNext() == trailer ){ throw new NodeOutOfBoundsException("Your reached the last node."); }
		return ((DNode<E>)target).getNext(); 
	}

	public Node<E> getNodeBefore(Node<E> target) throws NodeOutOfBoundsException {
		if(length == 0){ throw new NodeOutOfBoundsException("The list is empty"); }
		if(((DNode<E>)target).getPrev() == header ){ throw new NodeOutOfBoundsException("Your reached the first node."); }
	...
	public void removeNode(Node<E> target) {
		DNode<E> targetNode = (DNode<E>) target;
		DNode<E> targetNext = (DNode<E>) targetNode.getNext();
		DNode<E> targetPrev = (DNode<E>) targetNode.getPrev();
	...
	public void makeEmpty() { 
		while(header.getNext() != trailer){
			this.removeNode(header.getNext());
		}
	}
